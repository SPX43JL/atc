...
class Qwen2Attention(nn.Module):
    ...
    def forward(
        ...
        if (os.environ.get("ATC_KV_FAKE_QUANT_METHOD", "").lower() == "kvquant"
                and os.environ.get("ATC_KVQUANT_PREROPE", "0").lower()
                in {"1", "true", "yes", "on"}):
            from vllm.attention.ops.atc_kv_fake_quant import (
                maybe_kvquant_prerope_key)
            k = maybe_kvquant_prerope_key(k, positions, self._atc_attn_prefix)
