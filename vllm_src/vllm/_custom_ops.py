...
def reshape_and_cache(
    ...
    block_size = _atc_cache_block_size(key_cache, value_cache)
    key, value = maybe_fake_quant_kv(
        key, value, kv_cache_dtype, slot_mapping, block_size)
    torch.ops._C_cache_ops.reshape_and_cache(key, value, key_cache,
                                             value_cache, slot_mapping,
                                             kv_cache_dtype, k_scale, v_scale)
    maybe_cachewide_fake_quant_kv(key, value, key_cache, value_cache,
...
def _atc_cache_block_size(key_cache: torch.Tensor,
                          value_cache: torch.Tensor) -> int:
    if key_cache.ndim == 4:
